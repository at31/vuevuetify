<template>
   <v-card>
      <v-card-row class="green lighten-3">
         <v-card-title><span class="">Подтвердить заявку</span></v-card-title>
      </v-card-row>
      <v-card-text>
      <v-layout row>
            <v-flex xs4>
               <v-subheader>Отделение</v-subheader>
            </v-flex>
            <v-flex xs8>
               <v-text-field
                  name="postalCode"
            ...
            <v-flex xs8>
               <v-dialog
                  ...
                  <v-text-field  slot="activator"          
                     ...
                     prepend-icon="event"
                     readonly
                     :value="newEvent.start.format('DD-MM-YYYY')"
                     ></v-text-field>
...
export default {
    ...
    created() {
        this.newEvent = Object.assign({}, this.$store.state.radar.currEvent);
